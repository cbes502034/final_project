...
| # | 方法 | 路徑 | 權限 | 用途 |
|---|---|---|---|---|
| 1 | POST | `/api/auth/register` | 公開 | 註冊。回傳 user + tokens |
| 2 | POST | `/api/auth/login` | 公開 | 登入。回傳 access + refresh token |
| 3 | POST | `/api/auth/refresh` | 公開 | 用 refresh token 換新的 access token |
| 4 | POST | `/api/auth/logout` | 登入 | 撤銷目前的 refresh token |
| 5 | POST | `/api/auth/logout-all` | 登入 | 撤銷所有裝置的 token |
| 6 | GET | `/api/auth/me` | 登入 | 目前使用者、家庭角色、被誰監管 |
| 7 | PATCH | `/api/auth/password` | 登入 | 修改密碼，同時讓其他 session 失效 |
| 8 | GET | `/api/auth/sessions` | 登入 | 列出有效的登入裝置 |
...
| # | 方法 | 路徑 | 權限 | 用途 |
|---|---|---|---|---|
| 9 | GET | `/api/family` | 登入 | 家庭資訊、成員清單、角色 |
| 10 | POST | `/api/family` | 登入 | 建立家庭，建立者成為 master |
| 11 | POST | `/api/family/invite` | master | 產生邀請碼 |
| 12 | POST | `/api/family/join` | 登入 | 用邀請碼加入家庭 |
| 13 | PATCH | `/api/family/members/{userId}` | master | 修改成員角色 |
| 14 | DELETE | `/api/family/members/{userId}` | master | 移除成員（標記 removed，不刪資料） |
| 15 | GET | `/api/guardianships` | 登入 | 監管關係。**被監管者也看得到** |
| 16 | POST | `/api/guardianships` | master | 建立監管關係 |
| 17 | DELETE | `/api/guardianships/{id}` | master | 解除監管（設 `ended_at`，不刪除） |
...
| 23 | POST | `/api/categories` | master | 新增家庭自訂分類 |
...
（跟 MineMarket 把系統切成購物車／驗證／商品／管理者是同一個概念。）

| 成員 | 領域 | 資料表 | API | 畫面 | 該模組的 LLM ★ |
|---|---|---|---|---|---|
| **成員1** | **帳號與權限** | `users` `sessions` `families` `family_members` `guardianships` | 註冊登入登出、家庭、邀請碼、監管關係 | 註冊與登入、成員與權限 | **共用的模型呼叫層**：結構化輸出約束、重試、信心度計算 |
| **成員2** | **記帳** | `transactions` `categories` `accounts` `nlp_parses` | 明細 CRUD、`nlp/parse`、`nlp/parse-batch`、`nlp/confirm` | 段落記帳、單筆手動、缺欄位提示 | **段落切分策略、欄位抽取 prompt、低信心判準** |
| **成員3** | **統計與預算** | `budgets` `savings_goals` | `summary`、`stats`、`budgets`、`savings-goal` | 我的總覽、家庭總覽、統計圖表、超支警告 | **超支說明文案、分類體系定義** ⚑ |
| **成員4** | **財務建議** | `advices` | `advices`、`advices/generate` | 建議卡片、依據可驗算 | **建議生成 prompt、邊界規則、全系統評測** |

**這樣切的三個好處：**

1. **責任清楚** —— 某個畫面壞了，知道找誰
2. **可以平行動工** —— 四個人各改各的檔案，衝突少
3. **每個人都碰得到 LLM** —— 這是任務的硬性要求，垂直切片自然做到，
   不需要另外指派一個「模型負責人」
...
| 週次 | 主題 | 四個模組各自做什麼 |
|---|---|---|
| **第 1 週** | **前後端整合完畢** | 各自把「資料表 → API → 畫面」串通。這週結束時系統要能跑，只是還沒有模型 |
| **第 2 週** | **LLM 製作** | 各分支在自己的模組裡接上模型，做自己那部分的 prompt 與微調 |
| **第 3 週** | **LLM 改善／加強功能** | 評測、迭代。行有餘力再加功能 |

**第 1 週的重點是「串通」不是「做完」。** 只要每個模組都能從畫面按下去、
打到 API、寫進資料庫、再顯示出來，這週就算成功。功能細節可以粗糙。

## 6-3　第 1 週要凍結的三份契約

| 契約 | 誰定 | 誰要用 |
|---|---|---|
| **分類體系** ⚑ | 成員3 | 成員2 寫進 prompt、成員4 做建議時要用 |
| **API 回應形狀** | 各模組自己定，但要寫進 `js/api.js` | 其他人接畫面時要照著 |
| **解析結果的 JSON Schema** | 成員2 + 成員1 | 成員1 的模型呼叫層要照這個驗 |

**分類體系是唯一的跨模組阻塞點**，其他兩份各模組自己就能定。
