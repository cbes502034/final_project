...
| # | 方法 | 路徑 | 權限 | 用途 |
|---|---|---|---|---|
| 24 | POST | `/api/nlp/parse` ★ | 登入 | **只解析，不寫入**。文字 → 結構化欄位 + 各欄信心度 |
| 25 | POST | `/api/nlp/confirm` ★ | 登入 | 使用者確認／修正後寫入，同時記錄修正供評測 |
...
| 週次 | 後端（冠文） | 前端（宇傑） | 模型（明樺） | 領域（囷洧） |
|---|---|---|---|---|
| **W1** | 資料表 + Alembic + auth 三支路由 | 接 auth，做登入登出畫面 | 建立評測流程與零樣本基線 | **分類體系定稿**（其他人的前置） |
| **W2** | transactions + categories + permission guard | 接記帳與明細 | few-shot prompt 迭代 | 標註 200 筆、prompt 範例 |
| **W3** | ★ nlp/parse + nlp/confirm、stats、budgets | 接一句話記帳與統計 | 完整評測、對照表 | 建議文案準則、人工評分 |
| **W4** | advices、Docker、部署 | 接建議頁、Demo 演練 | 評測報告定稿 | 驗收與文件 |

**第 1 週要凍結的三份契約：**

1. **分類體系**（囷洧）→ 明樺訓練用、冠文寫進 prompt
2. **API 路由與回應形狀**（冠文）→ 宇傑接前端用
3. **解析結果的 JSON Schema**（冠文 + 囷洧）→ 三人共用
