...
def parse_one(body: ParseIn, me: User, db: Session = Depends(get_db)):
    ...
    【前端怎麼打】
        ...
        這支回 501（還沒做）、503（模型叫不動）時，前端會用自己的規則解析頂著，畫面照常能用。
    ...
    【完整寫法】照下面三步改，改完這支就做好了
        ...
        第二步：把整個 parse_one（從 @router.post 到 raise not_ready 那行）換成這段。
...
def parse_batch(body: ParseIn, me: User, db: Session = Depends(get_db)):
    ...
    【前端怎麼打】
        ...
        這支回 501、503 時，前端會用自己的規則切分頂著（寫入還是走後端）。
    ...
    【完整寫法】照下面三步改，改完這支就做好了
        ...
        第二步：把整個 parse_batch（從 @router.post 到 raise not_ready 那行）換成這段。
        注意 @stub 拿掉了；這段說明字串可以留著。
...
def confirm_one(body: ConfirmIn, me: User, db: Session = Depends(get_db)):
    ...
    【完整寫法】照下面兩步改，改完這支就做好了
        ...
        第二步：把整個 confirm_one（從 @router.post 到 raise not_ready 那行）換成這段。
...
def confirm_batch(body: ConfirmBatchIn, me: User, db: Session = Depends(get_db)):
    ...
    【完整寫法】照下面兩步改，改完這支就做好了
        ...
        第二步：把整個 confirm_batch（從 @router.post 到 raise not_ready 那行）換成這段。
        注意 @stub 拿掉了、多了 status_code=201；這段說明字串可以留著。
