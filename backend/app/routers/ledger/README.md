...
真的長什麼樣。你那一支還沒做的時候，**畫面上要用到它的那一塊**會直接說
「後端還沒做這一支（HTTP 501）」、是哪一支、誰負責；同一頁其他區塊照常能用。
做完一支、重新整理，**同一個位置自動變回正常內容**，前端一行都不用改。

⚠️ 前端不會替你補任何東西：那一支沒做，畫面上就不會出現對應的內容，
也不會拿假資料頂著。所以「畫面上有東西」就等於「你那一支真的回對了」。
...
| `[你的]` | 跑**路由檔裡真的寫的那一支**。還標著 `@stub` 時自動跳過，拿掉之後就開始跑 |

所以做完一支之後，只要看 `[你的]` 那半邊有沒有綠就好：

```bash
pytest tests/routes -k u4f60 -q
```

⚠️ `-k` 裡**不能直接打中文**。pytest 會把中文標籤轉成跳脫碼，實際的名字是 `[你的]`，
所以 `-k "你的"` 一個都比不到，只會印出 `0 selected`。要比就用 `u4f60`。
只驗一支就再加上函式名稱：`pytest tests/routes -k "create_group and u4f60" -v`
...
**還是卡住**：把畫面上那張通知卡的內容貼到群組（上面有函式、路由、狀態碼、負責人），
或是把 `uvicorn` 那個視窗最後幾行貼出來。
