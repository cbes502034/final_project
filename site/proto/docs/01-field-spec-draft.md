...
**但有大約四分之一可以救**：這 127 則裡有 **34 則（27%）的 CPE 已經帶了結構化邊界**，
也就是「人從描述文字看不出來，但 NVD 已經幫你標好了」。
剩下 **93 則（73%）兩邊都沒有**，那才是真正的「原文未提供」。
...
|---|---|---|---|
| 1 | 描述文字抽得到 | 抽取結果 | `text` |
| 2 | 抽不到，但 CPE 有邊界 | CPE 的值 | `cpe` |
| 3 | 兩邊都沒有 | 「原文未提供」 | `none` |

**建議加一個 `source` 欄位記錄值是哪裡來的。** 評測時要分開算：
來自 `text` 的才計入抽取模型的成績，來自 `cpe` 的不算模型的功勞。
不分開的話，模型看起來會比實際上準。
